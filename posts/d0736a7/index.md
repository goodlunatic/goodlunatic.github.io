...
我这里是随便把图片的高度调了一个比较大的值发现就可以看到下面的字符

但是其实我们也可以根据crc32爆破得到图像正确的高度：1023
...
通过解析JNG的参数，我们可以知道这个PNG的参数：`宽高与JPG一致、色深为1、色彩类型是灰度`（具体分析在下面原理详解里）

因此我们可以手动把IDAT的数据先提取出来，这里提取的时候要注意两个点

第一个就是JDAT前面的四字节是数据长度，需要删除

第二个就是JPG数据的后面四字节是crc校验，也需要删除

然后构造PNG头以及IHDR chunk，修个crc即可恢复出IDAT chunk中的内容，如下所示

![](imgs/image-20260116110546995.png)

![](imgs/image-20260116110605999.png)

修复完成后即可得到下面的这张PNG

![](imgs/image-20260116110659812.png)

把两张图片合起来看，删去黑色框框遮掉的内容即可得到最后的flag：`DASCTF{fab6c57e311163b0c93dee6ac65e8ce3}`
...
> 00 00 03 00 高度 
> 
> 0E 色彩类型，14对应Color-alpha，即带Alpha通道的彩色 
> 
> 08 采样深度，8对应8-bit 
> 
...
第一种是根据已知宽高、色深（1）、色彩类型（灰度）构造PNG文件头和IHDR chunk，将其转为灰度图像处理（详见上面的常规解）
...
![](imgs/image-20260131221821166.png)
...
因此尝试把所有图片最中心那个 3x3 的区域都提取并绘制到一张图片中了
...
## [SOLVED] 题目名称 XORkitt (2025CCF车联网专项赛/2025强网杯车联网专项赛)

> 这道题被复用了两场比赛，并且在两场比赛中都是 0 解题
> 
> 复用的两场比赛分别是2025CCF车联网专项赛/2025强网杯车联网专项赛
> 
> 这道题的成功解出离不开`@八神`和`@snowywar雪殇` 两位师傅的帮助，在此感谢一下二位师傅
...
后来赛后在`@八神`师傅的提示一下，知道了这种一行行分布的蓝色像素其实是那个双图盲水印工具的特征之一

> @八神：原图和盲水印后的图像异或会得到这种横向蓝色虚线是那个盲水印工具的特征之一

因此我们结合题目名的提示，把原图和其末尾的那张图片，用 stegsolve 的图像合成器异或一下

![](imgs/image-20251102135905666.png)


然后用 [这个项目](https://github.com/chishaxie/BlindWaterMark) 提取一下盲水印即可

![](imgs/image-20251102140300340.png)

![](imgs/image-20251102140247227.png)
...
## [SOLVED] 题目名称 hardstego_1（绿盟某比赛）

附件给了一张篡改过的PNG

![](imgs/image-20251216190143616.png)

010打开提示报错，发现是PNG后面几个chunk的type被删除了

![](imgs/image-20251216190228072.png)

直接改成IDAT并保存即可恢复原图

![](imgs/image-20251216190328051.png)

![](imgs/image-20251216191022289.png)

然后用 stegsolve 打开，发现 RGB plane 0 隐写了一个 zip

![](imgs/image-20251216190438927.png)

直接提取出来，发现是加密的，解压需要密码，然后 Red plane 0 中给了解压密码的提示

![](imgs/image-20251216190348330.png)

很明显是要我们去爆破一下5字节的CRC32

![](imgs/image-20251216190540159.png)

![](imgs/image-20251216190555276.png)

爆破得到`5h5hy74lny`，按理说这个应该就是上面压缩包的解压密码，但尝试后发现解压失败

后来在`@老铜匠`师傅的奇思妙想下，发现需要把里面的`l`改成`1`

即正确的解压密码是`5h5hy741ny`，解压后即可得到最后的flag: `flag{ef70dd3ee51878d315a67db591b35105}`

> 题目本身不难，但是问的师傅比较多，就想着记录一下
> 
> 就是这题的最后一步实在是出的不好，其实都可以说是错题了
> 
> 我猜应该是出题人在压缩那个加密压缩包的时候密码是手打的，结果打错了。。


## [SOLVED] 题目名称 像素流量（2025 上海市赛初赛）
...
后来在`@元亨利贞`师傅的帮助下，知道了这最后一步要先 From Hex，然后转为8位二进制

最后把得到的二进制数据每7位转为ASCII即可得到解压密码：

`D3f3nd_th3_r34lm_0f_1nf1n1ty!!!!`

> 把流量里的base64都提出来，能发现解码结果长度都是7的倍数

![](imgs/image-20260129185637708.png)
...
后来在`@元亨利贞`师傅的帮助下，知道了这里这一步考察的是`恩尼格玛(Enigma)密码机`

用在线网站填入对应参数解密即可得到解压密码：`PYZQFSBHBWFNALDYMAWW`

https://www.dcode.fr/enigma-machine-cipher

![](imgs/image-20260129192957502.png)

解压后得到`random.txt`，其中的内容如下：

```

```


![](imgs/image-20260129193409205.png)

base58解码后得到以下内容：

```
Here are five values generated by random functions. Can you crack the next value

0.8244267632622235
0.39195628838064667
0.6105479202635373
0.8210896449917546
0.017858540138089962
?

Submit flag with 32-bit lower case MD5 value
```

意思就是要我们根据五个随机数来预测第六个随机数

可以参考这个项目来爆破：https://github.com/PwnFunction/v8-randomness-predictor
...
def read_encode_mat(mat_path):
    ...
    # 打印关键信息（维度/长度、数据类型）
    print(f"FAO 维度: {fao.shape} | 数据类型: {fao.dtype}")
    print(f"M 长度: {len(M)} | 取值范围: {M.min()} ~ {M.max()}")
    print(f"N 长度: {len(N)} | 取值范围: {N.min()} ~ {N.max()}")

if __name__ == "__main__":
    read_encode_mat('encode.mat')

# FAO 维度: (512, 512, 3) | 数据类型: complex128
# M 长度: 256 | 取值范围: 1 ~ 256
# N 长度: 512 | 取值范围: 1 ~ 512
```
...
结合 original.jpg 这张图片的信息：图片宽高为`512x512` 并且是 `RGB` 的

猜测FAO变量里的数据就是原图变换后的图像数据，因此我们可以写个脚本提取出来
...
def extract_fao(mat_path, output_name):
    # 加载mat文件数据
    data = scipy.io.loadmat(mat_path)
    fao_data = data['FAO']
    
    # 1. 提取实部（空间域图像主要存在于实部）
    fao_real = np.real(fao_data)
    
    # 2. 转换为 16-bit 整数 (0 - 65535) 以保留精度
    # 不做数值拉伸，避免精度损失
    fao_16bit = np.clip(fao_real * 65535, 0, 65535).astype(np.uint16)
    
    # 3. 颜色通道转换 (RGB -> BGR)（适配OpenCV的通道顺序）
    fao_bgr = cv2.cvtColor(fao_16bit, cv2.COLOR_RGB2BGR)
    
    # 4. 使用无损压缩参数保存 PNG（压缩等级0表示无损）
    cv2.imwrite(output_name, fao_bgr, [cv2.IMWRITE_PNG_COMPRESSION, 0])
    print(f"图像已提取至: {output_name}")

if __name__ == "__main__":
    extract_fao('encode.mat','fixed.png')
```

提取出来的图像我们肉眼上看是和 `original.jpg` 一模一样的，但是这张图像是带有水印的

后续的解题思路参考自 [2021春秋杯新年欢乐赛-snowww](https://writeup.ctfhub.com/Challenge/2021/%E6%98%A5%E7%A7%8B%E6%9D%AF%E6%96%B0%E5%B9%B4%E6%AC%A2%E4%B9%90%E8%B5%9B/hDwx4fCmKfJ8UxrsstY4vw.html) （感谢`@秋雨渐冷`师傅提供的思路）

但是本题没有提供 `encode.m` 这个脚本，具体添加水印的脚本如下：
...
def extract_watermark(image_path, mat_path, alpha=80):
    # 1. 加载置乱索引
    data = sio.loadmat(mat_path)
    M = data['M'].flatten().astype(int) - 1 # 转换为0起始索引
    N = data['N'].flatten().astype(int) - 1

    # 2. 读取原始图像
    im_bgr = cv2.imread('original.jpg')
    im_rgb = cv2.cvtColor(im_bgr, cv2.COLOR_BGR2RGB).astype(np.float64) / 255.0

    # 3. 将16位BGR转为RGB，并还原回0.0-1.0的浮点数
    snow_16bit = cv2.imread(image_path, cv2.IMREAD_UNCHANGED)
    snow_rgb = cv2.cvtColor(snow_16bit, cv2.COLOR_BGR2RGB)
    snow = snow_rgb.astype(np.float64) / 65535.0

    # 4. FFT变换（添加移位）
    FA = np.fft.fft2(im_rgb, axes=(0, 1))
    FA = np.fft.fftshift(FA, axes=(0, 1))
    
    FB = np.fft.fft2(snow, axes=(0, 1))
    FB = np.fft.fftshift(FB, axes=(0, 1))

    # 5. 提取差异
    mark_diff = (FB - FA) / alpha
    mark_diff = np.fft.ifftshift(mark_diff, axes=(0, 1))

    # 6. 还原置乱
    rows, cols, ch = im_rgb.shape
    half_rows = rows // 2
    TH = mark_diff[0:half_rows, 0:cols, :]
    TH1_recovered = np.zeros((half_rows, cols, ch), dtype=np.complex128)

    # 双重循环还原位置
    for i in range(half_rows):
        for j in range(cols):
            TH1_recovered[M[i], N[j], :] = TH[i, j, :]

    # 7. 取幅值并归一化
    watermark_raw = np.abs(TH1_recovered)
    
    # 8. 增强显示（保留更多细节）
    final_watermark = np.zeros_like(watermark_raw, dtype=np.uint8)
    
    for c in range(3):
        channel = watermark_raw[:, :, c]
        # 使用更宽松的百分位裁剪（0.5% 到 99.5%）
        low, high = np.percentile(channel, (0.5, 99.5))
        if high > low:
            channel = np.clip(channel, low, high)
            channel = (channel - low) / (high - low) * 255
        final_watermark[:, :, c] = channel.astype(np.uint8)

    # 9. 保存结果
    result_bgr = cv2.cvtColor(final_watermark, cv2.COLOR_RGB2BGR)
    cv2.imwrite('flag.png', result_bgr)

if __name__ == "__main__":
    extract_watermark('fixed.png', 'encode.mat', alpha=80)
```
...
`DASCTF{2e93109f359f57a76807696be7b0180c}`
...
> 以下内容我没有在本地复现成功，不知道是附件有问题还是我的夜神模拟器有问题
> 
> 如果有师傅复现出来了，可以联系我交流一下，以下内容仅供参考
...
## 题目名称 简单的流量分析（2025泰山杯山东省网络安全大赛C组）

> 题目附件：https://pan.baidu.com/s/1oKf3w4UC3NKim8Lbypqfeg?pwd=96m8 提取码: 96m8

本题应该是一共有三问，前两问流量分析的过程可以参考这篇文章：[从一个流量样本学习webshell及CS流量的解密与分析](https://xz.aliyun.com/news/18953)


| ![](imgs/image-20260520104850408.png) | ![](imgs/image-20260520104901541.png) |
| :-----------------------------------: | :-----------------------------------: |

上面的那篇文章里只有前两问的题面，缺少第三问的题面，但是看最后拿到的那张图片的信息，猜测第三问是和图片隐写有关的

前两问详见上面那篇文章，既然已经有解，文本这里就不赘述了，以下内容仅针对第三问
...
> 题目附件：https://pan.baidu.com/s/1rcjqCfAUQQIKA-7XPN4jpQ?pwd=a4y1 提取码: a4y1

解压附件压缩包，得到一个加密的`flag.zip`和一个`passwd.gif`

gif文件存在帧间隔隐写，可以使用如下命令提取出帧间隔

```bash
identify -format "%s %T \n" passwd.gif
```

发现一共就三种情况：`10 30 60`，猜测可能是摩斯电码

因此我们把`10 -> .` `30 -> -` `60 -> 空格`，解摩斯电码

![](imgs/image-20260526144853305.png)

就是这里要注意的一点就是，直接得到的字母大写的字符串并不是解压密码

经过尝试发现，需要把这个字符串中的字母转小写才是正确的解压密码：`a3g5j87hg`

解压后可以得到 `pic0.png` - `pic45.png` 一共46张PNG

![](imgs/image-20260526145157635.png)

每张图片都挺大的，有5.5MB左右，并且放大后发现存在可疑的蓝色像素

![](imgs/image-20260526145314171.png)

一共46张PNG，猜测一张PNG可能对应flag中的一个字符

接下来我们就需要具体去分析每张PNG中的像素，去获取每张图片隐写的内容

浏览这46张图片发现一个很明显的特征：`pic45.png`中的蓝色像素到中间就截断了

![](imgs/image-20260526145746167.png)

根据这个特征，我们可以猜测这个像素是按行的规律隐写的

拿PS打开PNG，发现这个蓝色像素的具体RGB值是：`(12,123,234)`

![](imgs/image-20260526150414757.png)
...
解压附件压缩包可以得到一个加密的`？？？.rar`，尝试爆破发现解压密码是`ISCC`

解压后可以得到一个 `不知道啥玩意儿.png` 和一个加密的 `心律.rar`

010打开，发现PNG图片其实是一个纯文本文件

![](imgs/image-20260520110824844.png)

把其中所有用双引号包裹的内容提取出来，可以得到`心律.rar`的解压密码：`计划cludecorrectKEY（注释：大）`

解压后可以得到一个心律目录，里面有10个json文件

![](imgs/image-20260520111023550.png)
...
010打开，发现这张PNG末尾还藏了另一张PNG，手动提取出来即可得到下图
...
|               2025年的题面                |                    2026年的题面                    |
| :-----------------------------------: | :--------------------------------------------: |
| ![](imgs/image-20260520111602740.png) | ![](imgs/3a99fc7e5bb7ee5a803f17ff2ee33d44.png) |

> 这道题ISCC已经连续用了三年：2022 2025 2026，但是三年都是0解

解压附件可以得到一个 `可疑的.zip` 和一张 `相框里的图片.png`

zsteg扫一下这张PNG可以得到一个提示：`thekeyisYRSVJdW1xJBXlBbso?`

手动分割一下：`the key is YRSVJdW1xJBXlBb so?`

![](imgs/image-20260520111746943.png)

zip解压后可以得到一个 `碎片真相_REAL` 文件夹，其中包含的文件如下：

![](imgs/image-20260520111933332.png)

其中 `Ludovico Einaudi-Una Mattina描述.txt` 中的内容是：`oooe`

然后从频谱图中可以得到：`b921ac7b1f6da8ed4a2d041d4e2fbcf8`
