...
![](imgs/image-20250909185618290.png)
...
很明显解密这个密文需要密钥，后来听去了线下颁奖的师傅说，主办方在赛后总结的时候说这道题的密钥和具体的**经纬度**有关系

因此去网上搜了**天安门广场**的具体经纬度如下：
...
并且尝试用`116233954`作为 AES 的密钥解密成功，得到如下内容：

&gt; 23,71,5,6,97,1,53,5,5,7,6,7,97,6,1,5,7,4,101,1,53,1,8,9,4,19,37,2,1,97,107,7,1,97,47,103,47,2,3

![](imgs/image-20250909185149294.png)

```python
data1 = [58, 20, 36, 40, 32, 60, 48, 88, 42, 46, 70, 21, 42, 6, 51, 71, 40, 14, 30, 4, 37, 25, 28, 7, 39, 46, 20, 33]

data2 = [23, 71, 5, 6, 97, 1, 53, 5, 5, 7, 6, 7, 97, 6, 1, 5, 7, 4, 101, 1, 53, 1, 8, 9, 4, 19, 37, 2, 1, 97, 107, 7, 1, 97, 47, 103, 47, 2, 3]  

print(len(data1)) # 28
print(len(data2)) # 39
```

发现得到的两段数据或者说是接头暗号的长度是不一样的，然后回头看之前得到的信息，发现还有个时间没有用到，因此尝试获取了一下这个时间点的时间戳
