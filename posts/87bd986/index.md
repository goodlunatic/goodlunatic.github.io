...
翻看流量包，发现有很多个从`remote()`发到`localhost()`的数据包
...
后面解密的步骤就需要一点脑洞了，需要对数字进行一些猜测和尝试

![](imgs/image-20250420222336708.png)

当我们去除换行符，并且把0作为空格分隔符后

可以发现数据就剩下了 `11` `111` `22` `222` `44` `444` `88` `888` 这几种

&gt; 因为一共就剩下了1248这四种数字，所以我们根据做题经验会往四进制转换上联想

我们尝试在`CyberChef`中把两位三位的数字都替换成一位，并转换到四进制数上后可以得到如下数据
...
然后我们写一个脚本，两位为一组转成四进制可以得到 `6 6 6 12`，这刚刚好对应flag开头的`666c`
...
for i in range(0,len(data),2):
    tmp.append(int(data[i:i&#43;2],4))
print(tmp)
# [6, 6, 6, 12, 6, 1, 6, 7, 7, 11, 3, 5, 6, 1, 3, 4, 3, 8, 3, 2, 6, 1, 6, 4, 6, 5, 2, 13, 3, 1, 3, 0, 6, 5, 6, 4, 2, 13, 3, 4, 3, 9, 3, 0, 3, 5, 2, 13, 6, 1, 3, 8, 3, 8, 3, 6, 2, 13, 3, 3, 3, 6, 3, 9, 3, 8, 3, 4, 3, 6, 6, 4, 3, 2, 3, 7, 6, 1, 3, 0, 3, 8, 7, 13]

for item in tmp:
    res &#43;= f&#34;{item:x}&#34;
print(res)
# 666c61677b35613438326164652d313065642d343930352d613838362d3336393834366432376130387d
print(bytes.fromhex(res))
# b&#39;flag{5a482ade-10ed-4905-a886-369846d27a08}&#39;
```

因此我们再转为十六进制，然后转字符串即可得到最后的flag：`flag{5a482ade-10ed-4905-a886-369846d27a08}`
